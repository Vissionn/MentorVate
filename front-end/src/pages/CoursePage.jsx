...
import { CourseDetailHandler, fetchCourseDetails } from '../Services/operation/CourseDetailsApi'
...
const CoursePage = () => {
  ...
  // console.log("CourseId",id);
    ...
    const HandleBuyCourse = async() => {
        try {
          if(token) {
            const result = await BuyCourseHandler([id],user,dispatch,navigate,token)
            return
          } else {
            setConfirmationModal({
              ...
              btn1text: "Login",
              btn2text: "Cancel",
              bntHandler1: () => navigate("/login"),
              bntHandler2: () => setConfirmationModal(null),
            })
          }
    ...
    useEffect(() => {
      (async() => {
        ...
        if(res) {
          ...
          console.log("Could not fetch the course",error.messgae);
    ...
    useEffect(() => {
      (() => {
        try {
          const result =  GetAvgRating(response?.data?.courseDetail?.ratingAndReviews)
          if(result) {
    ...
    useEffect(() => {
      let lectures = 0;
      response?.data?.courseDetail?.courseContent.forEach((section) => {
    ...
    const handleAddToCart = () => {
      ...
      if (token) {
        dispatch(addToCart(response?.data?.courseDetail))
        return
      }
      setConfirmationModal({
        text1: "You are not logged in!",
  ...
  return (
    <div>
     <div className={`relative w-full bg-richblack-800`}>
      ...
      {/* whatyouwill learn and course-content section */}
